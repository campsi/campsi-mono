...
module.exports = class DocsService extends CampsiService {
    initialize() {
        ...
        this.router.param('resource', param(options));
        ...
        return new Promise((resolve) => {
            ...
            async.eachOf(options.resources, function (resource, name, cb) {
                ...
                deref(
                    resource.schema,
                    {'baseFolder': optionsBasePath, 'failOnMissing': true},
                    function(err, derefSchema) {
                        if(typeof err === 'undefined') {
                            resource.schema = derefSchema;
                            resource.collection = db.collection(name);
                            resource.validate = ajv.compile(derefSchema);
                        } else {
                            debug(err.message);
                            debug(resource);
                        }
