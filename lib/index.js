...
module.exports = class DocsService extends CampsiService {
    initialize() {
        ...
        const optionsBasePath = this.config.optionsBasePath;
        const db = this.db;
        ...
        this.router.param('resource', param.attachResource(options));
        this.router.param('resource', (req, res, next) => {
        ...
        return new Promise((resolve) => {
            ...
            async.eachOf(options.resources, function (resource, name, cb) {
                Object.assign(resource, options.classes[resource.class]);
                resource.collection = db.collection('docs.{0}.{1}'.format(config.namespace, name));
                $RefParser.dereference(optionsBasePath+'/', resource.schema, {})
