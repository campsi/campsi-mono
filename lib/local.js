...
module.exports.encryptPassword = function (password, saltRounds) {
    return new Promise(((resolve, reject) => {
        bcrypt.genSalt(saltRounds || 2, function (err, generatedSalt) {
            if (err) return reject({message: 'can not generate salt', detail: err});
            bcrypt.hash(password, generatedSalt, function (err, hash) {
                if (err) return reject({message: 'can not hash', detail: err});
                resolve(hash);
            });
        });
    }));
...
module.exports.signup = function (req, res) {
    ...
    module.exports.encryptPassword(req.body.password).then(encryptedPassword => {
        ...
        if (req.user) { // todo review use case
            ...
            users.insertOne(user)
                ...
                .catch((err) => {
                    debug(err);
                    handlers.redirectWithError(req, res, 'signup error, existing username');
                });
        }
        ...
        return helpers.error(res, {
            ...
            details: passwordEncryptionError
        });
    });
};
