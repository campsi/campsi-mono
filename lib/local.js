...
module.exports.signup = function (req, res) {
  ...
  const insertUser = function (user) {
    return users.insertOne(user)
      .catch((err) => {
        debug(err);
        handlers.redirectWithError(req, res, new Error('signup error, existing username'));
      });
  };
  const updateExistingNonLocalUser = function (user) {
    return new Promise((resolve, reject) => {
      users.findOneAndUpdate({email: user.email, 'identities.local': {$exists: false}}, {
        $set: {
        ...
        if (err) {
          return handlers.redirectWithError(req, res, new Error('could not perform findOneAndUpdate'));
        }
        resolve(result.lastErrorObject.updatedExisting);
      });
    });
  };
  ...
  module.exports.encryptPassword(req.body.password).then(encryptedPassword => {
    ...
    updateExistingNonLocalUser(user)
      .then(updatedExisting => updatedExisting ? null : insertUser(user))
...
module.exports.createResetPasswordToken = function (req, res) {
  ...
  req.db.collection('__users__').findOneAndUpdate(
    ...
    return helpers.error(res, {detail: err});
  });
};
