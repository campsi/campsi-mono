  ...
  return new Promise((resolve, reject) => {
    ...
    if (!isAggregationCursor) {
      ...
      /*
        ...
        Note: I could have used the code below for both types of cursor (Cursor and AggregationCursor), but since count() doesn't perform the query, I guessed that it would be better for performances to handle the count differently
       */
      cursor
        .toArray()
        .then(documents => {
          const count = documents.length;
          const info = preparePagination(count, skip, limit, page, perPage);
