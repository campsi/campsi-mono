 ...
 * @param {?Object}  countDocumentOptions CountDocumentsOptions
 * @param {?Object}  aggregateOptions AggregateOptions
 * @param {?integer} count  allow to bypass count query if a custom count has been done earlier
 ...
 */
module.exports.paginateQuery = async (
  ...
  if (count === null) {
    ...
    if (!isAggregation) {
      ...
      const aggregateCount = await collectionConnection
        .aggregate([...query, { $group: { _id: null, count: { $sum: 1 } } }], { allowDiskUse: true, ...aggregateOptions })
        .toArray();
