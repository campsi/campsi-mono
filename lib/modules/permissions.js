...
module.exports.can = function can (user, resource, method, state) {
  return new Promise((resolve, reject) => {
    if (typeof user === 'undefined') {
      ...
      return isAllowedTo(publicPermissions, method) ? resolve({}) : reject(new Error(PUBLIC_ERR_MESSAGE));
    }
    if (user.isAdmin) {
      return resolve({});
    }
...
module.exports.getAllowedStatesFromDocForUser = function (user, resource, method, doc) {
  const userRoles = doc.users[user._id].roles;
  let allowedStates = [];
  userRoles.forEach(role => {
    ...
    allowedStates.concat(statesForRole.filter(stateName => isAllowedTo(permsForRole[stateName], method)));
  });
