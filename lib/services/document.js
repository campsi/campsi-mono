...
// Helper functions
const getDocUsersList = (doc) => Object.keys(doc.users).map(k => doc.users[k]);
...
module.exports.getDocument = function (resource, schema, filter, query, user, state) {
  const requestedStates = getRequestedStatesFromQuery(resource, query);
...
module.exports.setDocumentState = function (resource, filter, fromState, toState, user) {
  return new Promise((resolve, reject) => {
    ...
    resource.collection.findOne(filter, (err, document) => {
      ...
      if (allowedPutStates.includes(stateTo.name) && allowedGetStates.includes(stateFrom.name)) {
