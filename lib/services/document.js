...
module.exports.getDocuments = function(resource, schema, user, query, state, sort, pagination) {
    ...
    const dbQuery = builder.find(queryBuilderOptions);
    ...
    return new Promise((resolve, reject) => {
        cursor.count()
            .then((count) => {
                ...
                let {skip, limit, page} = paginateCursor(cursor, pagination);
                result.hasNext = result.count > skip + limit;
                result.hasPrev = skip > 0;
                result.perPage = pagination.perPage;
                result.page = page;
                result.pageCount = Math.ceil(count / pagination.perPage);
                sortCursor(cursor, state, sort);
